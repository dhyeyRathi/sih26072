...
export function ForecasterDesk({ open, onClose, onPendingCountChange }: ForecasterDeskProps) {
  ...
  const broadcast = async (alertId: string) => {
    ...
    try {
      ...
      setStatus(`Broadcast simulated: ${body.broadcast?.total_recipients ?? 0} recipients.`);
    } catch (error) {
  ...
  if (!open) return null;
  return (
    <aside className="absolute right-4 top-16 z-40 flex max-h-[calc(100%-6rem)] w-[26rem] flex-col overflow-hidden rounded-xl border border-slate-700/70 bg-[#0a0e27]/95 shadow-2xl backdrop-blur-xl">
      <header className="flex items-start justify-between border-b border-slate-700/60 p-4"><div><h2 className="flex items-center gap-2 font-bold text-white"><ShieldCheck className="h-5 w-5 text-emerald-300" /> Forecaster warning desk</h2><p className="mt-1 text-xs text-slate-400">Human review remains required before dissemination.</p></div><button onClick={onClose} className="rounded p-1 text-slate-400 hover:bg-slate-800 hover:text-white"><X className="h-5 w-5" /></button></header>
